...
def get_dataset(dataset_name, batch_size, n_worker, data_root='data', for_inception=False):
    ...
    if dataset_name == 'ILSVRC2012':
    ...
    elif dataset_name == 'cifar10-res':
        normalize = transforms.Normalize(mean=[0.491, 0.482, 0.447], std=[0.247, 0.243, 0.262])   #res20
        # normalize = transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))    #res18
        ...
        trainset = torchvision.datasets.CIFAR10(root='./data', train=True, transform=train_transform, download=False)
        testset = torchvision.datasets.CIFAR10(root='./data', train=False, transform=test_transform, download=False)
        train_loader = torch.utils.data.DataLoader(dataset=trainset, batch_size=batch_size, shuffle=True, num_workers=n_worker)
        val_loader = torch.utils.data.DataLoader(dataset=testset, batch_size=batch_size, shuffle=False, num_workers=n_worker)
        n_class = 10
    else:
...
def get_split_train_dataset(dataset_name, batch_size, n_worker, val_size, train_size=None, random_seed=1,
    ...
    if dataset_name == 'imagenet':
        ...
        traindir = os.path.join(data_root, 'imagenet_train')
        valdir = os.path.join(data_root, 'val_ccq')
        assert os.path.exists(traindir), traindir + ' not found'
        assert os.path.exists(valdir), valdir + ' not found'
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])
