...
class QModule(nn.Module):
    def __init__(self, w_bit=-1, a_bit=-1, ratio=1, half_wave=True):
    ...
    def _compute_divergence(self, data, bitwidth):
        ...
        div_channel_out = min(d)
        # th_layer_out = threshold[np.argmin(d)]
        # sf_layer_out = scaling_factor[np.argmin(d)]
        # threshold = float(th_layer_out)
        return div_channel_out
    ...
    def _quantize_activation(self, inputs):
        ...
        if self._quantized and self._a_bit > 0:
            ...
            # print(self._trainable_activation_range, self._half_wave) 
    ...
    def _quantize_weight(self, weight):
        ...
        if self._quantized and self._w_bit > 0:   
            ...
            dsp_number = math.floor(weight.shape[0] * self.ratio)   #weight batch number = output channel number
            bismo_number = weight.shape[0] - dsp_number            
            # print('dsp:',dsp_number, 'bismo:',bismo_number)
            
            if self.channel_type is None:
                self.channel_type = [0] * weight.shape[0]
            ...
            if self._calibrate:
                ...
                for (idx, channel) in enumerate(weight):        
                    # if self._w_bit < 5 or self.channel_type[idx] == 1:
                    threshold = self._compute_threshold(channel.data.cpu().numpy(), self._w_bit)    #most time-consuming
            ...
            for (idx, channel) in enumerate(weight):            
                ...
                scaling_factor = threshold / (pow(2., temp_w_bit - 1) - 1.)
                # scaling_factor = threshold / 127
                new_ch = ori_ch.clamp(-threshold, threshold)
                # w[w.abs() > threshold - threshold / 64.] = 0.
                new_ch.div_(scaling_factor).round_().mul_(scaling_factor)
                # print(np.unique(new_ch.reshape(-1).detach().cpu().numpy()).shape)
                self.new_w[idx] = new_ch
...
class QConv2d(QModule):
    ...
    def forward(self, inputs):
        ...
        # print(np.unique(inputs[0].reshape(-1).detach().cpu().numpy()).shape)
        # print(inputs.max())
        
        # print("+++++++++++++++++ {} ++++++++++++++++".format((weight == self.weight).all())) 
        # hist, bin_edges = np.histogram(inputs.detach().cpu().numpy(), bins=256, range=(0, inputs.detach().cpu().numpy().max()), density=True)
        # print(hist)
...
def calibrate(model, loader):
    ...
    inputs, _ = next(iter(loader))  # only first batch
    # print(inputs.numel(), torch.sum(inputs > 0))
