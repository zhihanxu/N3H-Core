...
class QModule(nn.Module):
    ...
    def _compute_threshold(self, data, bitwidth):
        ...
        if np.isclose(mx, 0.0):
            return 0.0
        hist, bin_edges = np.histogram(np.abs(data), bins='sqrt', range=(mn, mx), density=True) #density:True return prob of each range
        hist = hist / np.sum(hist) 
        cumsum = np.cumsum(hist)    
        n = pow(2, int(bitwidth) - 1)   # level numbers
        threshold = []
        scaling_factor = []
        d = []
        ...
        for i in range(n + 1, len(bin_edges), 1):
            ...
            q = np.copy(p)
            q[:i] = q_interp
            d_tmp = np.sum((cumsum - q) * np.log2(cumsum / q))  # Kullback-Leibler-J    sum(p[i] * log2(p[i]/q[i])
    ...
    def _compute_divergence(self, data, bitwidth):
        mn = 0
        mx = np.abs(data).max()
        if np.isclose(mx, 0.0):
            return 0.0
        ...
        th_channel_out = min(d)  # min
        return th_channel_out
    ...
    def _quantize_activation(self, inputs):
        ...
        if self._quantized and self._a_bit > 0:
            if self._calibrate:
                ...
                #     # estimate_activation_range = self.init_range
                # else:
                estimate_activation_range = min(self.init_range, inputs.abs().max().item())
                    # estimate_activation_range = inputs.abs().max().item()
    ...
    def _quantize_weight(self, weight):
        if self._quantized and self._w_bit > 0:   
            ...
            if self._calibrate:
                bismo_number = int(round(weight.shape[0] * self.ratio))   #weight batch number = output channel number
                dsp_number = int(weight.shape[0] - bismo_number)            
                # print('dsp:',dsp_number, 'bismo:',bismo_number)
                ...
                #computing divergence/variance
                for (idx, channel) in enumerate(weight): 
                    # divergence = self._compute_divergence(channel.data.cpu().numpy(), self._w_bit)
                    # all_div.append(divergence)
                    variance = torch.var(channel)
                    all_div.append(variance)
                    
                all_div_copy.extend(all_div)
                all_div.sort()
                temp_div.extend(all_div)
                
                if bismo_number > 0:
                    div_threshold = temp_div[bismo_number-1]
                    for idx in range(weight.shape[0]):
                        if all_div_copy[idx] < div_threshold or all_div_copy[idx] == div_threshold:
                            self.channel_type.append(1) #bismo
                        else:
                            self.channel_type.append(0) #dsp
                else:
                    self.channel_type = [0]*dsp_number
                   
                for (idx, channel) in enumerate(weight):        
                    # if self._w_bit < 8 and self.channel_type[idx] == 1:
                    #     threshold = self._compute_threshold(channel.data.cpu().numpy(), self._w_bit)
                    # else:
                    threshold = channel.abs().max().item()
                
                # print("+++++++++++++++++ {} ++++++++++++++++".format((ori_w == self.new_w).all()))  
                return weight

            ori_w = weight
            self.new_w = ori_w.clone().detach()
            ...
            for (idx, channel) in enumerate(weight):            
                ...
                scaling_factor = threshold / pow(2., temp_w_bit - 1)
                ori_ch = ori_ch.clamp(-threshold, threshold)               
                sign = ori_ch.sign()
                new_ch = torch.log2(torch.abs(ori_ch/scaling_factor)).round() - 2
                new_ch = sign * torch.round(pow(2, new_ch)) * scaling_factor
                
                # scaling_factor1 = threshold / pow(2., round(temp_w_bit/2) - 1)
                # ori_ch = ori_ch.clamp(-threshold, threshold)               
                # sign = ori_ch.sign()
                # new_ch1 = torch.log2(torch.abs(ori_ch/scaling_factor1)).round() - 2
                # new_ch1 = sign * torch.round(pow(2, new_ch1)) * scaling_factor1  
                # x = ori_ch/scaling_factor1 - new_ch1
                # sign2 = x.sign()
                # scaling_factor2 = threshold / pow(2., np.floor(temp_w_bit/2) - 1)
                # new_ch2 = torch.log2(torch.abs(x/scaling_factor2)).round() - 2 
                # new_ch2 = sign2 * torch.round(pow(2, new_ch2)) * scaling_factor2
                # new_ch = (new_ch1 + new_ch2)
                
                # print('ori_ch: \n', ori_ch)
                # print('new_ch: \n', new_ch)
                
                self.new_w[idx] = new_ch


            # STE
            if self._fix_weight:    #false
                # w = w.detach()
                return self.new_w.detach()
            else:
                # w = ori_w + w.detach() - ori_w.detach()
                return STE.apply(weight, self.new_w)  
...
def calibrate(model, loader):
    ...
    inputs = inputs.to('cuda:2', non_blocking=True)
    # print(inputs.shape)
    with torch.no_grad():
        model(inputs)
    for name, module in model.named_modules():
