...
class QModule(nn.Module):
    ...
    def _quantize_weight(self, weight):
        ...
        if self._quantized and self._w_bit > 0:
            ...
            if self._calibrate:
                self.channel_type = rand(bismo_number, dsp_number)    # 0 dsp, 1 bismo 
                    
                for (idx, channel) in enumerate(weight):
                    if self._w_bit < 5 and self.channel_type[idx] == 1: #当bismo位宽大于5，精度不会变self._w_bit < 5 and 
...
def rand(num1, num2):
    ...
    for i in range(num1+num2):
        val = int(random.choice(seed))
        if val == 1 and count < num1:
            lis.append(val)
            count = count + 1
        elif val == 0 and count2 < num2:
            lis.append(0)
            count2 = count2 + 1
        elif count2 >= num2:
            lis.append(1)
