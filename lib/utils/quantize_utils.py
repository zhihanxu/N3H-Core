...
class QModule(nn.Module):
    ...
    def _quantize_activation(self, inputs):
        if self._quantized and self._a_bit > 0:
            ...
            if self._trainable_activation_range:
                if self._half_wave:
                    ori_x = 0.5 * (inputs.abs() - (inputs - self.activation_range).abs() + self.activation_range)   #PACT
                else:
            ...
            scaling_factor = self.activation_range.item() / (2. ** self._a_bit - 1.)    #无负数
    ...
    def _quantize_weight(self, weight):
        ...
        if self._quantized and self._w_bit > 0:
            threshold = self.weight_range.item()    #initial: -1.0
            if threshold <= 0:
                threshold = weight.abs().max().item() 
                self.weight_range.data[0] = threshold   #initialize
            
            if self._calibrate: #calculate threshold
                if self._w_bit < 5:
...
def calibrate(model, loader):
    ...
    inputs = inputs.to('cuda:3', non_blocking=True)
    with torch.no_grad():
        model(inputs)
    for name, module in model.named_modules():
        if isinstance(module, QModule):
