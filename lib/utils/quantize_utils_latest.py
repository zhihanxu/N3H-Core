...
class QModule(nn.Module):
    ...
    def _quantize_weight(self, weight):
        ...
        if self._quantized and self._w_bit > 0:   
            ...
            if self._calibrate:
                bismo_number = round(weight.shape[0] * self.ratio)   #weight batch number = output channel number
                dsp_number = weight.shape[0] - bismo_number            
                ...
                if bismo_number > 0:
                    if self._a_bit > self._w_bit:
                        div_threshold = temp_div[bismo_number-1]
                        for idx in range(weight.shape[0]):
                            if all_div_copy[idx] < div_threshold or all_div_copy[idx] == div_threshold:
                                self.channel_type.append(1) #smaller divergence BISMO
                            else:
                                self.channel_type.append(0) #larger divergence
                    else:
                        div_threshold = temp_div[dsp_number-1]
                        for idx in range(weight.shape[0]):
                            if all_div_copy[idx] < div_threshold or all_div_copy[idx] == div_threshold:
                                self.channel_type.append(0) #smaller divergence DSP
                            else:
                                self.channel_type.append(1) #larger divergence
                else:
                    self.channel_type = [0]*dsp_number
                   
                for (idx, channel) in enumerate(weight):        
                    if (self._w_bit < 8 and self.channel_type[idx] == 1) or (self.channel_type[idx] == 0 and self._a_bit == 4):
