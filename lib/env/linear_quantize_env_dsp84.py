...
class LinearQuantizeEnv:
    ...
    def step(self, action):
        ...
        self.last_weight_action = self._action_wall(action)
        ...
        self.layer_embedding[self.cur_ind][-1] = self.last_ratio_action
    ...
    def _build_state_embedding(self):
        ...
        if self.is_imagenet:
            measure_model(self.model_for_measure, 128, 128)
        else:
        ...
        for i, ind in enumerate(self.quantizable_idx):
            ...
            if type(m) == nn.Conv2d or type(m) == QConv2d:  #conv layer
                this_state.append([int(self.cur_ind==0 or self.cur_ind==20)])  # layer type, 1 for conv_dw
