...
from lib.utils.quantize_utils_DSPfirst import QConv2d, QLinear, calibrate
...
class LinearQuantizeEnv:
    def __init__(self, model, pretrained_model, data, data_root, compress_ratio, args, n_data_worker=16,
        ...
        self.action_radio_button = 1    #1:ratio 0:weight -1:output

        self.is_inception = args.arch.startswith('inception')
        self.is_imagenet = ('imagenet-data' in data)
    ...
    def step(self, action):
        ...
        if self.action_radio_button != -1:
            action = self._action_wall(action)  # percentage to preserve    不是ratio, 采取离散化action

        if self.action_radio_button == 1:   #weight
            self.last_weight_action = action    
        elif self.action_radio_button == 0: #activation
            self.last_activation_action = action
        else:
            self.last_ratio_action = round(float(action),2) #ratio
            self.strategy.append([self.last_weight_action, self.last_activation_action, self.last_ratio_action])  # save action to strategy
            self.acc_ratio += self.last_ratio_action
            self.acc_ratio = round(self.acc_ratio,2)
            self.index += 1
            # print(self.acc_ratio, self.index)
        
        # all the actions are made
        if self._is_final_layer() and (self.action_radio_button == -1): #ratio:-1    1 0 -1
            self._final_action_wall()   #before after action wall
            assert len(self.strategy) == len(self.quantizable_idx)
            cost = self._cur_cost()
            cost_ratio = cost / self._org_cost()

            self.avg_ratio = self.acc_ratio / self.index
            print('avg ratio:', self.avg_ratio)
            
            self._set_mixed_precision(quantizable_idx=self.quantizable_idx, strategy=self.strategy)
            self.model = calibrate(self.model, self.val_loader)   #train_loader
            if not self.finetune_flag:                                  # default true
                ...
                # default 1, 每个layer finetune 1个epoch
                ...
                acc = self._validate(self.val_loader, self.model)   #validate
        ...
        if self.action_radio_button == -1:
            self.cur_ind += 1   #build index of next layer
        else:
    ...
    def reward(self, acc, cost_ratio=None):
        if cost_ratio is not None:
            return (acc - self.org_acc + 1. / cost_ratio + 1. /avg_ratio) * 0.1
    ...
    def _final_action_wall(self):
        target = self.compress_ratio * self._org_cost() #compress相当于compress了cost
        ...
        print('\nbefore action_wall: ', self.strategy, 'min_cost:', min_cost, 'target:', target, 'current_cost:', self._cur_cost())
        while min_cost < self._cur_cost() and target < self._cur_cost():
            ...
            for i, n_bit in enumerate(reversed(self.strategy)):
                ...
                # if n_bit[-1] > 0.05:
                #     self.strategy[-(i+1)][0] -= 0.05
                # self._keep_first_last_layer()
                # if target >= self._cur_cost():
                #     break
        print('after action_wall: ', self.strategy, 'min_cost:', min_cost, 'target:', target, 'current_cost:', self._cur_cost())
    ...
    def _action_wall(self, action):
        ...
        # action = action/10  # ratio 0.2-0.8
    ...
    def _build_index(self):
        ...
        for i, m in enumerate(self.model.modules()):
            if type(m) in self.quantizable_layer_types: #问题 无
    ...
    def _get_lookuptable(self):
        ...
        if self.cost_mode == 'cloud_latency':
            fname = lookup_table_folder + 'qmobilenetv2_imagenet100_batch16_latency_table.npy'
